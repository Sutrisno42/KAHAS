...
const ChartsWidget4: React.FC<Props> = ({ className, title }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${API_URL}/report/financial?type=monthly`);
        setChartData(response.data.data.summary_year);
      } catch (error) {
...
function getChartOptions(height: number, data: any[]): ApexOptions {
  ...
  return {
    series: [
      // {
      //   name: 'Net Profit',
      //   data: [44, 55, 57, 56, 61, 58, 50, 45, 58, 64,
      //     55, 63, 59, 55, 52, 61, 65, 70, 74, 68,
      //     61, 56, 53, 50, 48, 47, 49, 52, 57, 62],
      // },
      {
        name: 'Profit',
